...
# Named comparisons, so every figure in RETHINKING_SUPERSAT can be regenerated by
# name rather than by editing this file. Each entry lists the run directories (as
# ``(directory, legend label)`` pairs, drawn in order with the colours below), the
# output stem written into PARENT, and the figure title.
COMPARISONS = {
    # The original S3/Δμ=0 vs. equilibrium-baseline comparison.
    "s3_d0_vs_baseline": {
        "runs": [
            ("BASELINE_JHOM_35", "Baseline (no inert states)"),
            ("X_320_Y_80_3.0_D0.0_JHOM_-3.50_F0.0_K1.0", r"S3, $\Delta\mu=0$"),
        ],
        "stem": "comparison_S3_D0_vs_baseline",
        "title": r"Undriven S3 vs. equilibrium baseline  ($J_{\rm hom}=-3.5$, $J_{\rm het}=-2.0$)",
    },
    # Effect of switching the S3 drive on at otherwise identical parameters:
    # same lattice, same couplings, Δμ = 0 vs Δμ = 0.5.
    "s3_drive": {
        "runs": [
            ("X_320_Y_80_3.0_D0.0_JHOM_-3.50_F0.0_K1.0", r"S3, $\Delta\mu=0$"),
            ("X_320_Y_80_3.0_D0.5_JHOM_-3.50_F0.0_K1.0", r"S3, $\Delta\mu=0.5$"),
        ],
        "stem": "comparison_S3_D0.5_vs_D0.0",
        "title": r"S3 driving strength: $\Delta\mu=0$ vs. $\Delta\mu=0.5$  "
                 r"($320\times80$, $J_{\rm hom}=-3.5$, $J_{\rm het}=-2.0$)",
    },
}

# Colours applied to the runs of a comparison, in order.
COLORS = [BLUE, RED, "#4c9a2a", "#8b5aa8"]


def run_comparison(key: str, parent: Path = PARENT) -> pd.DataFrame:
    """Load, summarize and plot one named comparison from :data:`COMPARISONS`."""
    spec = COMPARISONS[key]
    runs = [
        load_run(parent / directory, label=label, color=color)
        for (directory, label), color in zip(spec["runs"], COLORS)
    ]

    summary = summarize(runs)
    print(summary.to_string(index=False))
    summary.to_csv(parent / f"{spec['stem']}.csv", index=False)

    compare_runs(
        runs,
        outfile=parent / f"{spec['stem']}.png",
        title=spec["title"],
    )
    return summary
...
if __name__ == "__main__":
    ...
    argparser.add_argument(
        "comparison",
        nargs="?",
        default="s3_d0_vs_baseline",
        choices=sorted(COMPARISONS),
        help="which named comparison to regenerate",
    )
