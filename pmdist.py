...
hist2, edges2 = np.histogram(compare_data, bins=500, range=(0, 1), density=True)
bin_centers2 = 0.5 * (edges2[1:] + edges2[:-1])
hist2 = hist2+ 1e-10
print(hist2)

ax.plot(bin_centers2, hist2, label="Reference PDF ordered")


ax.plot(bin_centers, hist, label="Reference PDF disordered")
ax.set_yscale('log')
ax.set_xlabel(r'$m$')
ax.set_ylabel(r'$p(m)$')
